...
function findRowsNeedingHubDBCreation(allData, sheet, changes) {
  ...
  changedUniqueIds.forEach(uniqueId => {
    ...
    if (!data) return;
    const hasRequiredData = data.date_and_time && data.player_1 && data.player_2;
...
function detectChanges(currentData, storedData) {
  ...
  Object.keys(currentData).forEach(uniqueId => {
    ...
    if (!stored) {
      ...
      const changedFields = [];
      const fieldsToCheck = ['date_and_time', 'category', 'round', 'pitch', 'player_1', 'player_2', 'winner'];
...
function sendToHubSpot(changes, sheet) {
  try {
    ...
    // Clear sync status for all rows that will be processed
    ...
    // Separate DELETE operations from others
    ...
    // Handle batch deletes if we have multiple DELETE operations
    if (deleteChanges.length > 1) {
      ...
      deleteChanges.forEach(change => {
        ...
        const batchResponseCode = batchResponse.getResponseCode();
        const batchResponseText = batchResponse.getContentText();
        
        if (batchResponseCode >= 200 && batchResponseCode < 300) {
          const batchResponseData = JSON.parse(batchResponseText);
          console.log('Successfully batch deleted from HubSpot');

          // Update status for deleted changes
            ...
            deleteChanges.forEach(change => {
              if (change.oldData?.sheetRow) {
                ...
                if (wasDeleted) {
                  ...
                  sheet.getRange(change.oldData.sheetRow, SYNC_MESSAGE_ID_COLUMN).setValue("Successfully deleted from HubDB");
                } else {
                  sheet.getRange(change.oldData.sheetRow, SYNC_STATUS_ID_COLUMN).setValue("error");
                  sheet.getRange(change.oldData.sheetRow, SYNC_MESSAGE_ID_COLUMN).setValue("Batch deletion failed");
                }
              }
            });
          } else {
    ...
    if (otherChanges.length > 0) {
      ...
      const individualPayload = {
        gameData: otherChanges.map(change => ({
          ...
          metadata: {
            timestamp: new Date().toISOString(),
            source: 'google_sheets_sync',
            sheetRow: change.row || (change.oldData ? change.oldData.sheetRow : null)
          }
        }))
      };
      ...
      if (responseCode >= 200 && responseCode < 300) {
        ...
        // Update error status for all processed rows
        otherChanges.forEach(change => {
          const targetRow = change.row || (change.oldData ? change.oldData.sheetRow : null);
          if (targetRow) {
